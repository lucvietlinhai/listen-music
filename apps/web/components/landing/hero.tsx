const stats = [
  { label: "Người đang nghe", value: "2,184" },
  { label: "Phòng đang mở", value: "327" }
];
    ...
    <section className="mx-auto grid w-full max-w-6xl gap-8 px-4 pb-16 pt-14 md:grid-cols-2 md:items-center md:pt-20">
      <div>
        ...
        <h1 className="text-4xl font-extrabold leading-tight text-text sm:text-5xl">
          Cùng nghe một bài nhạc,{" "}
          <span className="text-gradient">cùng một khoảnh khắc</span>
        </h1>
        <p className="mt-5 max-w-xl text-base leading-relaxed text-muted sm:text-lg">
          Tạo phòng trong vài giây, mời bạn bè bằng một đường link, chat và thả cảm xúc
          theo thời gian thực. Không cần cài app.
        </p>
        <div className="mt-7 flex flex-wrap gap-3">
          <button className="rounded-xl bg-accent px-5 py-3 text-sm font-bold text-slate-950 transition hover:brightness-110">
            Tạo phòng ngay
          </button>
          <button className="rounded-xl border border-line bg-surface px-5 py-3 text-sm font-semibold text-text transition hover:border-accent">
            Tham gia phòng
          </button>
        </div>
      </div>
